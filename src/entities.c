...
void doEntities(void)
{
	...
	//for (e = stage.entityHead.next; e != NULL; e = e->next) // loops through entities
	//{
	//	if (e->touchingSpike != NULL) // checks if entity is touching the spikes
	//	{
...
static void moveToEntities(Entity *e, float dx, float dy)
{
	...
	for (other = stage.entityHead.next ; other != NULL ; other = other->next)
	{
		if (other != e && collision(e->x, e->y, e->w, e->h, other->x, other->y, other->w, other->h))
		{
			...
			else if (other->flags & EF_SOLID + EF_WEIGHTLESS + EF_DANGEROUS)
			{
				if (other->touch)
				{
					other->touch(e);
